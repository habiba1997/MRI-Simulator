...
class window(QtWidgets.QMainWindow):
    ...
    def Reconstruction (self):
        ...
        self.t1 = np.zeros((self.img.shape[0],self.img.shape[1]))
        self.t2 = np.zeros((self.img.shape[0],self.img.shape[1]))
        for i in range(self.img.shape[0]):
            for j in range(self.img.shape[1]):
                    self.t1[i][j] = self.createT1(self.img[i][j])
                    self.t2[i][j]  = self.createT2(self.img[i][j])
        self.ForLoops(Tr,Te,vector,Kspace)
    


    def ForLoops(self,TR,TE,vector,Kspace):    
        ...
        for i in range(self.pahntomForLoop.shape[0]):
            for j in range(self.pahntomForLoop.shape[1]):
                signal[i][j] =  self.rotationAroundYaxisMatrix(30,vector)
                signal[i][j] = signal[i][j] * np.exp(-TE/self.t2[i][j])
                signal[i][j] = self.recoveryDecayEquation(self.t1[i][j],self.t2[i][j],1,np.matrix(signal[i][j]),TR)
        ...
        #maxKspace = np.max(Kspace)
        #minKspace=np.min(Kspace)
        #Kspace = (Kspace - minKspace) * (255/(maxKspace - minKspace)) 
